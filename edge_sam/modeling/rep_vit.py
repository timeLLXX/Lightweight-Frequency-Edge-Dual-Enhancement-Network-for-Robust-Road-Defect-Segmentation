...
class RepViTBlock(nn.Module):
    ...
    @torch.no_grad()
    def fuse(self):
        # 处理 token_mixer 中的融合路径
        for name, sub in self.token_mixer.named_children():
            if hasattr(sub, 'fuse') and callable(sub.fuse):
                fused = sub.fuse()
                if fused is not None:
                    setattr(self.token_mixer, name, fused)

        # 处理 channel_mixer 中的融合路径
        if hasattr(self.channel_mixer, 'fuse') and callable(self.channel_mixer.fuse):
            fused = self.channel_mixer.fuse()
            if fused is not None:
                self.channel_mixer = fused

        # 可选：如果你希望返回替换模块（便于 setattr 替换）
        return self  # 返回自身表示“融合完毕”
